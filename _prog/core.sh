...
_create_ubDistBuild-bootOnce-qemu_sequence() {
	...
	_messagePlain_nominal 'wait: 3200s'
	local currentIterationWait
	currentIterationWait=0
	pgrep qemu-system
	pgrep qemu
	ps -p "$currentPID"
	while [[ "$currentIterationWait" -lt 3200 ]] && ( pgrep qemu-system > /dev/null 2>&1 || pgrep qemu > /dev/null 2>&1 || ps -p "$currentPID" > /dev/null 2>&1 )
	do
		sleep 1
		let currentIterationWait=currentIterationWait+1
	done
	_messagePlain_probe_var currentIterationWait
	[[ "$currentIterationWait" -ge 3200 ]] && _messagePlain_bad 'bad: fail: bootdisc: poweroff' && currentExitStatus=1
	sleep 27
...
_create_kde() {
	...
	#-T0
	env XZ_OPT="-e9" tar --exclude= './.ubtmp' --exclude='./.config/chromium' --exclude './.config/systemd/user/bootdiscStartup.service' --exclude='./.config/startup.sh' --exclude='./.config/autostart/startup.desktop' --exclude='./.config/plasma-workspace/env/startup.sh' --exclude'./.config/qt5ct' -cJvf "$scriptLocal"/package_kde.tar.xz ./.config ./.kde ./.local ./.xournal/config ./.license_package_kde
